...
class GeneralCatcher(val treshold: Double, val batchSize: Int, val minIter: Int, val maxIter: Int) extends ResultCatcher {
  ...
  override def isValid(scores: Array[Array[Double]]): Boolean = true
